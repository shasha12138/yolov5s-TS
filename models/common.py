...
class SE(nn.Module):
    def __init__(self, c1, c2, reduction=16):
        ...
        self.avgpool = nn.AdaptiveAvgPool2d(1)  # 全局自适应池化
        self.l1 = nn.Linear(c1, c1 // reduction, bias=False)
        self.relu = nn.ReLU(inplace=True)
        self.l2 = nn.Linear(c1 // reduction, c1, bias=False)
        self.sig = nn.Sigmoid()


    def forward(self, x):
        b, c, _, _ = x.size()
        y = self.avgpool(x).view(b, c)  # squeeze操作
        y = self.l1(y)
        y = self.relu(y)
        y = self.l2(y)
        y = self.sig(y)
        y = y.view(b, c, 1, 1)  # FC获取通道注意力权重，是具有全局信息的
